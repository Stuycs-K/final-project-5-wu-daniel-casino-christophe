...
public class PacMan{
  ...
  public void updateLocation(){    
    if(PacManDirection.equals(new PVector(0,-4))){
      if(PacManMap[getCurrentTile()[1] - 1][getCurrentTile()[0]] != 0){
        location.add(PacManDirection);
      }
    } else if(PacManDirection.equals(new PVector(0,4))){
      if(PacManMap[getCurrentTile()[1] + 1][getCurrentTile()[0]] != 0){
        location.add(PacManDirection);
      }
    } else if(PacManDirection.equals(new PVector(-4,0))){
  ...
  public void applyDirection(String direction){
    if (direction.equals("up")){
       if(PacManMap[getCurrentTile()[1] - 1][getCurrentTile()[0]] != 0){
        PacManDirection = new PVector(0,-4);
      }
       ...
       if(PacManMap[getCurrentTile()[1]][getCurrentTile()[0] - 1] != 0){
        PacManDirection = new PVector(-4, 0);
      }
